...
struct formatting_context
{
   ...
   bool inlined;
   bool withMarkup;
   formatting_context(formatting_type type) {
      this->inlined = type != formatting_type::Html;
      this->withMarkup = type != formatting_type::InlineText;
   }
   std::string get_style_id(const std::string& style);
...
struct formatting_options
{
   ...
   halign papAlign;
   bool papInTbl;
   formatting_options(formatting_context& _context) : context(_context)
   {
