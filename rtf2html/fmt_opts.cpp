...
{
   _ASSERT(this->context.withMarkup);
   std::string style;
   switch (papAlign)
   {
   ...
   {
      if (this->context.inlined) return std::string("<p style=\"") + style + "\">";
      else return std::string("<p class=\"") + this->context.get_style_id(style) + "\">";
   }
}

std::string formatting_context::get_style_id(const std::string &style)
{
...
std::string formatting_context::get_styles()
{
   std::string result;
...
{
   formatting_options last_opt(_opt.context), opt(_opt);
   std::string result;
   if (!opt_stack.empty())
   {
      int cnt=0;
      ...
      if (i!=opt_stack.rend())
      {
         while (cnt--)
         {
            if (opt.context.withMarkup) result+="</span>";
            opt_stack.pop_back();
         }
         return result;
      }
       ...
       && last_opt.chpVAlign!=opt.chpVAlign)
   {
      ...
      last_opt=opt_stack.empty()?formatting_options(_opt.context):opt_stack.back();
   }
   std::string style;
   ...
   if (!opt.context.withMarkup) return "";
   else if (opt.context.inlined) return result + "<span style=\"" + style + "\">";
   else return result + "<span class=\"" + opt.context.get_style_id(style) + "\">";
}
...
std::string formatter::close()
{
   ...
   for (fo_deque::iterator i=opt_stack.begin(); i!=opt_stack.end(); ++i)
      if (i->context.withMarkup) result += "</span>";
   return result;
}
