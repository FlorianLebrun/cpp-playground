...
namespace streamwriter {
   ...
   void int64_to_ascii(uint8_t* buf, int bufsize, int64_t value, int radix);
   void float64_to_ascii(uint8_t* buf, int bufsize, double value);
   ...
   struct stream_utf8 {
      ...
      stream_utf8& puti(int64_t value, int radix = 10) {
         uint8_t buf[64];
         int64_to_ascii(buf, sizeof(buf), value, radix);
         stream.write_codes<default_coding, uint8_t>(buf, buf + strlen((char*)buf));
         return *this;
      }
      stream_utf8& putf(double value) {
         uint8_t buf[64];
         float64_to_ascii(buf, sizeof(buf), value);
         stream.write_codes<default_coding, uint8_t>(buf, buf);
         return *this;
      }
   };
