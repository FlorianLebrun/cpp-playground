...
MemoryRegion32::MemoryRegion32(uint8_t regionID, bool restrictSmallPtrZone) {
   ...
   this->address = regionID << cRegionSizeL2;
...
void MemoryRegion32::print() {
   for (size_t i = 0; i < 1024; i++) {
      ...
      printf("%d-%d\t[%d]\t%s", this->regionID, i, size_t(1) + unit.width, status);
...
address_t getSegmentIndex(MemoryUnit* unit, size_t position, size_t multiplier) {
   address_t address = unit->address();
   address.segmentID += position * multiplier;
   return address;
}

address_t MemorySubunitManifold::acquireSubunitSpan(MemorySpace* space, size_t lengthL2) {
...
retry:
   ...
   for (intptr_t sizeID = lengthL2; sizeID < cBinPerManifold; sizeID++) {
      ...
      while (auto unit = bin.units) {
         ...
         if (position >= 0) {
            return getSegmentIndex(unit, position, this->packing);
         }
...
void MemorySubunitManifold::scavengeCaches(MemorySpace* space) {
   int checked_units = 0;
...
void MemorySubunitManifold::releaseSubunitSpan(MemorySpace* space, address_t address, size_t lengthL2) {
   auto region = space->regions[address.unit.regionID];
   auto unit = &region->units_table[address.unit.unitID];
   unit->fragments_bitmap.release(address.unit.segmentID, lengthL2);
   auto new_spare_metric = unit->fragments_bitmap.getSpareSizeL2();
...
MemorySpace::MemorySpace() {
   for (int packing = 1; packing <= 7; packing += 2) {
      this->subunits_manifolds[packing >> 1].packing = packing;
   }
   this->createRegion(1);
}
...
MemorySpace::~MemorySpace() {
   for (size_t i = 0; i < 256; i++) {
      if (auto region = this->regions[i]) {
         ...
         OSMemory::ReleaseMemory(uintptr_t(region), cSegmentSize);
         this->regions[i] = 0;
      }
   }
}
...
MemoryRegion32* MemorySpace::createRegion(size_t regionID) {
   ...
   _ASSERT(region);
   OSMemory::CommitMemory(uintptr_t(region), cSegmentSize);
...
address_t MemorySpace::acquireSegmentSpan(size_t packing, size_t shift) {
   if (shift < cSegmentPerUnitL2) {
      MemorySubunitManifold& manifold = this->subunits_manifolds[packing >> 1];
      return manifold.acquireSubunitSpan(this, shift);
   }
   else {
      auto unit = this->acquireUnitSpan(packing << (shift - cSegmentPerUnitL2));
      return unit->address();
   }
}

address_t MemorySpace::acquireSegmentSpan(size_t length) {
   const uintptr_t cMaxSpanSizeForFragmentedUnit = 7 << cSegmentPerUnitL2;

   if (length <= cMaxSpanSizeForFragmentedUnit) {
      size_target_t target(length);
      MemorySubunitManifold& manifold = this->subunits_manifolds[target.packing >> 1];
      return manifold.acquireSubunitSpan(this, target.shift);
   }
   else {
      auto unit = this->acquireUnitSpan(alignX(length, cSegmentPerUnit) >> cSegmentPerUnitL2);
      return unit->address();
   }
}

void MemorySpace::releaseSegmentSpan(address_t base, size_t length) {
   _ASSERT(length > 0);
   auto region = this->regions[base.unit.regionID];
   auto unit = &region->units_table[base.unit.unitID];
   if (unit->isFragmented) {
      size_target_t target(length);
      this->subunits_manifolds[target.packing].releaseSubunitSpan(this, base, target.shift);
   }
   else {
      region->releaseUnitSpan(unit, base.unit.segmentID);
   }
}

void MemorySpace::scavengeCaches() {
   for (int i = 0; i < cPackingCount; i++) {
      this->subunits_manifolds[i].scavengeCaches(this);
   }
}
